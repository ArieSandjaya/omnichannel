...
CREATE TABLE tenants (
  ...
  settings            JSONB        NOT NULL DEFAULT '{"theme":{"primary_color":"#6366f1","secondary_color":"#ec4899","hero_text":"Selamat Datang!","font_family":"Inter, sans-serif"},"store":{"address":"","phone":"","email":""}}',
  logo_url            TEXT,
...
CREATE POLICY tenant_isolation ON inventory_log
  FOR ALL USING (tenant_id = current_tenant_id());

CREATE POLICY inventory_log_immutable_update ON inventory_log FOR UPDATE USING (FALSE);
CREATE POLICY inventory_log_immutable_delete ON inventory_log FOR DELETE USING (FALSE);
